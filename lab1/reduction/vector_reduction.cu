...
runTest(int argc, char** argv) 
{
    ...
    //   host's result.
    // * array_mem_sizeOne argument: Read the input data array from the given file.
    switch(argc-1)
    {      
        case 1:  // One Argument
...
float computeOnDevice(float* h_data, int num_elements)
{
  ...
  reduction<<<1, NUM_ELEMENTS/2>>>(d_data, NUM_ELEMENTS);
  cudaMemcpy(h_data, d_data, num_elements*sizeof(float), cudaMemcpyDeviceToHost);

  char* filename="output.txt";
  cutWriteFilef(filename, h_data, NUM_ELEMENTS,0.0001f);
