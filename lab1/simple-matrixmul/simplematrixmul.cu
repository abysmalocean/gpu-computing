...
int main(int argc, char** argv) 
{
 ...
 // compute the matrix multiplication on the CPU for comparison
    ...
    char* referencename="reference.txt";
    WriteFile(reference, referencename); 
...
void MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
    //Interface host call to the device kernel code and invoke the kernel---launch kernel
    Matrix dM;
    Matrix dN;
    Matrix dP;

    dM.width = dM.height = dM.pitch = MATRIX_SIZE;
    dN.width = dN.height = dN.pitch = MATRIX_SIZE;
    dP.width = dP.height = dP.pitch = MATRIX_SIZE;
    dM.elements = dN.elements = dP.elements = NULL;
    
    cudaMalloc((void**)&(dM.elements), MATRIX_SIZE*MATRIX_SIZE*sizeof(float));
    cudaMalloc((void**)&(dN.elements), MATRIX_SIZE*MATRIX_SIZE*sizeof(float));
    cudaMalloc((void**)&(dP.elements), MATRIX_SIZE*MATRIX_SIZE*sizeof(float));

    CopyToDeviceMatrix(dM, M);
    CopyToDeviceMatrix(dN, N);
    CopyToDeviceMatrix(dP, P);

    dim3 threadID_Grid(16, 16);
    MatrixMulKernel<<<1,threadID_Grid>>>(dM, dN, dP);
