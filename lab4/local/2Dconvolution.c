...
int main(int argc, char** argv) 
{
	...
	printf("height of B is :%d\n",B.height);
	printf("Width of B is :%d\n",B.width);
...
void ConvolutionOnDevice(const Matrix A, const Matrix B, Matrix C)
{
	...
	dmb = clCreateBuffer(context, CL_MEM_READ_ONLY, (B.height+4)*(B.width+4)*sizeof(float), NULL, &status);
	printf("status is : %d\n",status);
	chk(status, "clCreateBuffer");
	...
	float* hb = (float*)malloc((B.height+4)*(B.width+4)*sizeof(float));
	int a,b;
	for( a=0;a<B.height+4;a++)
	{
		for(b=0;b<B.width+4;b++)
			hb[a*(B.width+4)+b]=0.0f;
	}
	for( a=0;a<B.height;a++)
	{
		for( b=0;b<B.width;b++)
			hb[(a+2)*(B.width+4)+b+2]=B.elements[a*B.width+b];
	}

	WriteToFile("hb",hb, B.height+4,B.width+4);
	...
	clEnqueueWriteBuffer(queue, dmb, CL_TRUE, 0, (B.height+4)*(B.width+4)*sizeof(float), hb, 0, NULL, NULL);
	printf("status is : %d\n",status);
	chk(status, "clEnqueueWriteBuffer");
	...
	//create program with source file
	...
	//assign the work-item and work-group dimension
	unsigned int global_work_size_0,global_work_size_1;
	int grp_size_row,grp_size_col,kernel_size,block_size;

	global_work_size_0 = roundup(C.width,BLOCK_SIZE);
	global_work_size_1 = roundup(C.height,BLOCK_SIZE);

	size_t localsize[2] ={BLOCK_SIZE,BLOCK_SIZE} ;
	size_t globalsize[2] ={global_work_size_0,global_work_size_1} ;
	size_t localmem = (BLOCK_SIZE+4)*(BLOCK_SIZE+4)*sizeof(float);	
	size_t locala = KERNEL_SIZE*KERNEL_SIZE*sizeof(float);

	grp_size_row=roundup(C.height,BLOCK_SIZE)/BLOCK_SIZE;
	grp_size_col=roundup(C.width,BLOCK_SIZE)/BLOCK_SIZE;
	...
	printf("Total work-item number in dimension 0(width) is:%d\n",global_work_size_0);
	printf("Total work-item number in dimension 1(height) is:%d\n",global_work_size_1);

	printf("Work group number in dimension 0 is : %d\n",grp_size_row);
	printf("Work group number in dimension 1 is : %d\n",grp_size_col);
	...
	printf("status is : %d\n",status);
	status|=clSetKernelArg(kernel, 4, locala, NULL);
	printf("status is : %d\n",status);
	status|=clSetKernelArg(kernel, 5, sizeof(int), &C.height);
	printf("status is : %d\n",status);
	status|=clSetKernelArg(kernel, 6, sizeof(int), &C.width);
	printf("status is : %d\n",status);
	status|=clSetKernelArg(kernel, 7, sizeof(int), &grp_size_row);
	printf("status is : %d\n",status);
	status|=clSetKernelArg(kernel, 8, sizeof(int), &grp_size_col);
	printf("status is : %d\n",status);
	status|=clSetKernelArg(kernel, 9, sizeof(int), &kernel_size);
	printf("status is : %d\n",status);
	status|=clSetKernelArg(kernel, 10, sizeof(int), &block_size);
	printf("status is : %d\n",status);
