...
void ConvolutionOnDevice(const Matrix A, const Matrix B, Matrix C)
{
	...
	clEnqueueWriteBuffer(queue, dmb, CL_TRUE, 0, B.height*B.width*sizeof(float),B.elements, 0, NULL, NULL);
	printf("status is : %d\n",status);
	chk(status, "clEnqueueWriteBuffer");
	...
	size_t globalsize[2] ={C.width,C.height} ;
	...
	printf("status is : %d\n",status);
	status|=clSetKernelArg(kernel, 3, sizeof(int), &C.height);
	printf("status is : %d\n",status);
	status|=clSetKernelArg(kernel, 4, sizeof(int), &C.width);
	printf("status is : %d\n",status);
	chk(status,"clSetKernekArg");

	cl_event event;
	//pass the work-item and work-group dimension to kernel
	status=clEnqueueNDRangeKernel(queue,kernel,2,NULL,globalsize,NULL,0,NULL,&event);
	printf("status is : %d\n",status);
	...
	printf("The height of Matrix B is:%d\n",B.height);
	printf("The width of Matrix B is :%d\n",B.width);	
