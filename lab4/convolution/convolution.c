...
int main(int argc, char** argv) 
{
	...
	d_filter = clCreateBuffer(context, CL_MEM_READ_ONLY, 49*sizeof(float),NULL, NULL);
	...
	clEnqueueWriteBuffer(queue, d_filter, CL_TRUE, 0, 49*sizeof(float), filter, 0, NULL, NULL);
	...
	// Execute the kernel
	build_status=clEnqueueNDRangeKernel(queue, kernel, 2, NULL, globalSize, localSize, 0, NULL, NULL);
	chk(build_status, "clEnqueueNDRange");
	...
	printf("Executation is %.5f\n",(float)st/CLOCKS_PER_SEC);

	int m;
	for(m=0;m<(deviceDataSize/sizeof(float));m++)
		printf("%8.3f",outputImage[m]);
	printf("\n");
