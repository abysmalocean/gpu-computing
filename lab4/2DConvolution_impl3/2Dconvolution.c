...
//check if every setup of OpenCL is right
void chk(cl_int status, const char* cmd)
{
...
void ConvolutionOnDevice(const Matrix A, const Matrix B, Matrix C)
{
	...
	//create command queue with profiling enable in order to measure the kernel execution time
	...
	float* paddingb = (float*)malloc((B.height+4)*(B.width+4)*sizeof(float));
	PaddingMatrixB(paddingb, B.elements, B.height, B.width);	
	...
	clEnqueueWriteBuffer(queue, dmb, CL_TRUE, 0, (B.height+4)*(B.width+4)*sizeof(float), paddingb, 0, NULL, NULL);
	printf("status is : %d\n",status);
	chk(status, "clEnqueueWriteBuffer");
	...
	size_t globalsize[2] ={roundup(C.width,BLOCK_SIZE),roundup(C.height,BLOCK_SIZE)} ;
